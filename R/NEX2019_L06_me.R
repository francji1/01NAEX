...
#library(dynsurf)  not available any more ??
#install.packages("dynsurf")
...
### Rest = GARBAGE - Burn after reading
...
Problem_61 <- read.table("Problem_6_31.txt",header=TRUE,sep=";")

m         <-  FrF2(2^4, 4, replications = 1, randomize = F,factor.names = c("A", "B","C", "D"))
Weight    <- Problem_61[1:16,"Weight"]
Viscosity <- Problem_61[1:16,"Viscosity"]
...
## Daniel plot and Half normal plot are usually used for unreplicatd design
# Daniel Plot with alpha = 0.1 and only significant factors
DanielPlot(m,response = "Weight", code=TRUE)
DanielPlot(m,response = "Viscosity", code=TRUE)


# Classical effects qqplot
qqline(DanielPlot(m,response = "Weight",alpha=0.05)$y)
qqline(DanielPlot(m,response = "Viscosity",alpha=0.05)$y)


# half normal plot of effects
DanielPlot(m,response = "Weight",code=TRUE,alpha=0.1,half=TRUE)
DanielPlot(m,response = "Viscosity",code=TRUE,alpha=0.1,half=TRUE)


# ANOVA and lm for Weight
anova(aov(Weight~A*B*C*D, data=m))  # No DF
anova(aov(Weight~A*C*B, data=m))
anova(aov(Weight~A+C+B+A:B, data=m))  # Final Anova with B - keep Hierarchy
summary(lm(Weight~A+C+B+A:B, data=m)) # corresponding lm model

m$A.num <- 2*(as.numeric(m$A)-1.5)
m$B.num <- 2*(as.numeric(m$B)-1.5)
m$C.num <- 2*(as.numeric(m$C)-1.5)
m$D.num <- 2*(as.numeric(m$D)-1.5)
W.lm       <- lm(Weight~A.num+C.num+A.num:B.num,data=m)
summary(W.lm)  # corresponding num. model in coded
...
m$D.num2         = 60
m$D.num2[m$D==1] = 75
W.lm2       <- lm(Weight~A.num2+C.num2+A.num2:B.num2,data=m)
summary(W.lm2)  # corresponding num. model in true variables

# ANOVA and lm for Viscosity
anova(aov(Viscosity~A*B*C*D, data=m))
anova(aov(Viscosity~A*B, data=m))
anova(aov(Viscosity~A+B, data=m)) # Final anova
summary(lm(Viscosity~A+B, data=m)) # final lm model

V.lm       <- lm(Viscosity~A.num+B.num,data=m)
summary(V.lm)  # corresponding num. model



tmp            <- list(A.num=seq(-1,1,by=.05),B.num=seq(-1,1,by=0.05),C.num=seq(-1,1,by=0.05),data=rate)
new.data       <- expand.grid(tmp)
new.data$fitW   <- predict(W.lm,new.data)
new.data$fitV   <- predict(V.lm,new.data)

contourplot(fitW~A.num+C.num,new.data,xlab="A Temperature",ylab="C Time", main = "Contour plot of chemical process 6.31")
contourplot(fitW~A.num+B.num,new.data,xlab="A Temperature",ylab="B Concentration", main = "Contour plot of chemical process 6.31")
contourplot(fitV~A.num+B.num,new.data,xlab="A Temperature",ylab="B Concentration", main = "Contour plot of chemical process 6.31")

a=seq(-1,1,by=.05)
b=seq(-1,1,by=0.05)
fitW2 = matrix(a[,1] + a[,2], 20,20)

with(new.data,filled.contour(A.num,C.num,fitW,xlab="A Temperature",ylab="C Time", main = "Contour plot of chemical process 6.31"))
